"""Product Agent - build product comparison matrix and product claims."""
...
def _llm_enabled() -> bool:
    value = os.getenv("PRODUCT_AGENT_USE_LLM", "1").strip().lower()
...
def _matrix_cell(
    ...
    if related:
        ...
        summary = f"基于{', '.join(evidence_ids[:3])}，{platform}在{dimension}上已有直接证据支撑：{first_claim[:80]}"
    else:
        summary = f"暂无{platform}在{dimension}上的直接证据，存在 evidence_gap，建议后续补充产品专项采集。"
...
你的职责：
- 只分析产品相关维度，例如硬件性能、软件驱动、用户口碑、产品线广度、产品体验、参数规格、生态兼容性。
...
- 如果 focus_dimensions 里包含定价策略等偏商业维度，只做轻量产品侧描述，不生成商业策略结论。
...
def _normalize_llm_matrix(
    ...
    matrix_payload = payload.get("product_matrix") or payload.get("matrix")
...
def _next_claim_id(existing_count: int, index: int) -> str:
    return f"PCL{existing_count + index:03d}"
...
def _claim_from_related(
    ...
    content = (
        f"{platform}在{dimension}维度有证据支持，主要体现为："
        f"{first_claim[:90]}"
    )
    return Claim(
        claim_id=claim_id,
    ...
    for dimension in dimensions:
        for platform in platforms:
            ...
            claim = _claim_from_related(
                claim_id=_next_claim_id(len(existing_claims), len(claims) + 1),
                platform=platform,
                dimension=dimension,
                related=related,
            )
...
def _normalize_llm_claims(
    ...
    for item in payload["claims"]:
        ...
        try:
            claim = Claim(
                claim_id=_as_text(item.get("claim_id")) or _next_claim_id(len(existing_claims), len(claims) + 1),
                content=_as_text(item.get("content")) or "产品维度结论缺少内容，已过滤为证据约束结论。",
            ...
            continue
        claim.claim_id = _next_claim_id(len(existing_claims), len(claims) + 1)
        claims.append(claim.model_dump())

    return claims or fallback_claims


def _append_trace(state: dict, product_claims: List[Dict[str, Any]]) -> None:
    ...
    trace_log.append(
        {
            ...
            "status": "success",
            "output_summary": f"generated product_matrix and {len(product_claims)} product claims",
            "error": None,
        }
    )


def product_agent(state: dict) -> Dict[str, Any]:
    """Analyze product dimensions and output a matrix plus product claims."""
    _load_env()
    evidence_list = [
    ...
    try:
        ...
        product_matrix = _normalize_llm_matrix(
            payload=llm_payload,
        ...
        product_claims = _normalize_llm_claims(
            payload=llm_payload,
    ...
    product_claim_dicts = [claim.model_dump() for claim in output.claims]
    ...
    _append_trace(next_state, product_claim_dicts)

    dimension_count = len(output.product_matrix.get("dimensions", {}))
    print(f"[ProductAgent] 产品矩阵生成完成，共 {dimension_count} 个维度")
    return next_state
