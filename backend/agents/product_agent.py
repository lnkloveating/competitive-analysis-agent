"""Product Agent - build product capability comparison matrix."""
...
def _fallback_cell(
    ...
    if related:
        ...
        summary = f"暂无{platform}在{dimension}上的直接证据，按现有公开信息保守估计，建议后续补充专项采集。"
...
def _extract_matrix_payload(payload: Any) -> Any:
    ...
    if isinstance(payload, dict):
        for key in ("product_matrix", "matrix", "data", "result"):
            if key in payload:
...
def product_agent(state: CompetitiveAnalysisState) -> CompetitiveAnalysisState:
    """Analyze product dimensions and output a platform comparison matrix."""
    _load_env()
    ...
    if not _llm_enabled():
        error_log.append("ProductAgent 已按 PRODUCT_AGENT_USE_LLM 配置跳过 LLM 调用，使用兜底产品矩阵。")
    elif os.getenv("ARK_EP") and os.getenv("ARK_API_KEY"):
        try:
            llm = get_llm()
        except Exception as exc:  # pragma: no cover - defensive guard for SDK init.
            error_log.append(f"ProductAgent 初始化 LLM 失败：{exc}")
    else:
        error_log.append("ProductAgent 未找到 ARK_EP 或 ARK_API_KEY，已启用兜底产品矩阵。")

    try:
        llm_payload = _generate_with_llm(llm, state)
        product_matrix = _normalize_matrix(llm_payload, state, fallback)
    except Exception as exc:
        error_log.append(f"ProductAgent 生成产品矩阵失败：{exc}")
        product_matrix = fallback

    dimension_count = len(product_matrix.get("dimensions", {}))
