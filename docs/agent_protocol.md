...
- 每个 Agent 有明确职责边界，只能读写自己负责的字段。
- 每个核心输出都经过 Pydantic Schema 校验。
- 最终报告必须引用已有 `claims` 和 `evidence_ids`。
- `QualityAgent` 可以结构化定向打回，例如打回 `EvidenceAgent` 或 `ProductAgent`。
...
- 旧 workflow state 已通过 reducer 合并这两个字段，避免并行分支覆盖彼此输出。
...
- 未来真实爬虫只需要实现 `ResearchProvider.collect(state)` 接口。
...
`ProductAgent` 只能分析产品相关维度，如硬件性能、软件驱动、用户口碑、产品线广度、产品体验等。
...
`BusinessAgent` 关注定价策略、渠道策略、市场定位、品牌定位、商业模式、产品线策略等。
...
```json
{
  ...
  "missing_dimensions": ["定价策略"],
  "missing_platforms": [],
  "required_actions": ["补充定价策略相关 evidence"],
  "checked_items": {
...
```json
{
  ...
  "status": "success",
  "output_summary": "generated 12 evidence items",
  "duration_ms": 1234,
  "error": null
}
```
